...
def main() -> None:
    ...
    df['density_group'] = df['population_density'].apply(
        lambda x: 'High Density (Urban Core)' if x > density_threshold else 'Low Density (Peri-urban)'
    )
    
    print(f"  ✓ Loaded {len(df)} records")
    print(f"  ✓ Density threshold: {density_threshold:.1f} people/km²")
    print(f"  ✓ High density: {(df['density_group'] == 'High Density (Urban Core)').sum()} records")
    print(f"  ✓ Low density: {(df['density_group'] == 'Low Density (Peri-urban)').sum()} records")
...
f"- **High Density (Urban Core):** Population density > {density_threshold:.1f} people/km²  ",
    ...
    for r in results:
        report_lines.append(f"| {r['Group']} | {r['Sample Count']} | {r['Precision']} | {r['Recall']} | {r['F1-Score']} |")
    ...
    if len(results) == 2:
        ...
        if f1_diff < 0.05:
            report_lines.append("**Assessment:** Model performance is **relatively consistent** across density groups, suggesting no significant bias in favor of urban or peri-urban areas. The classifier generalizes well across different infrastructure contexts.")
        elif f1_diff < 0.10:
            report_lines.append("**Assessment:** Model shows **minor performance variation** between density groups. This is within acceptable limits for real-world ML systems and may reflect genuine differences in data quality or feature distributions rather than systematic bias.")
        else:
            report_lines.append("**Assessment:** Model exhibits **noticeable performance disparity** between density groups. Further investigation recommended to ensure equitable predictive accuracy across urban and peri-urban counties.")
