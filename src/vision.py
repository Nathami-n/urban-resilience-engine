...
def simulate_ndvi_from_climate(features_df: pd.DataFrame) -> pd.DataFrame:
    ...
    annual_data = features_df.groupby(['county_id', 'year']).agg({
        'rainfall_mm': 'mean',
        'temp_max_c': 'mean',
    ...
    soc_norm = (annual_data['soil_organic_carbon'] - annual_data['soil_organic_carbon'].min()) / \
               (annual_data['soil_organic_carbon'].max() - annual_data['soil_organic_carbon'].min())
    ...
    annual_data['ndvi_std'] = 0.05 + 0.1 * (1 - rain_norm.values) + np.random.uniform(0, 0.03, size=len(annual_data))
    
    ndvi_df = annual_data[['county_id', 'year', 'ndvi_mean', 'ndvi_std', 'ndvi_obs_count']].copy()
    
    print(f"  ✓ Generated NDVI for {ndvi_df['county_id'].nunique()} counties × {ndvi_df['year'].nunique()} years")
    print(f"  ✓ NDVI range: {ndvi_df['ndvi_mean'].min():.3f} to {ndvi_df['ndvi_mean'].max():.3f}")
    print(f"  ✓ Mean NDVI: {ndvi_df['ndvi_mean'].mean():.3f} ± {ndvi_df['ndvi_mean'].std():.3f}")
    
    return ndvi_df


def merge_ndvi_to_features(features_df: pd.DataFrame, ndvi_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'ndvi_mean' in merged.columns:
        if merged['ndvi_mean'].isnull().any():
            print(f"  ⚠ Filling {merged['ndvi_mean'].isnull().sum()} missing NDVI values with mean")   
            merged['ndvi_mean'].fillna(merged['ndvi_mean'].mean(), inplace=True)
            merged['ndvi_std'].fillna(merged['ndvi_std'].mean(), inplace=True)
            merged['ndvi_obs_count'].fillna(merged['ndvi_obs_count'].median(), inplace=True)
    else:
        print(f"  ⚠ Warning: ndvi_mean column not found after merge")
        print(f"  Debug - ndvi_df columns: {list(ndvi_df.columns)}")
        print(f"  Debug - merge keys match: county_id={merged['county_id'].nunique()}, year={merged['year'].nunique()}")
...
def main() -> None:
    ...
    try:
        ...
        print("\n📊 NDVI Summary:")
        print(ndvi_df[['county_id', 'year', 'ndvi_mean']].groupby('county_id')['ndvi_mean'].describe())
