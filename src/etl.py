...
def generate_climate_data(counties_df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(
        f"  + Generated {len(climate_df)} climate records >> {chirps_dir}/climate_monthly.csv"
    )
...
def generate_soil_data(counties_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, county in counties_df.iterrows():
        ...
        base_soc = (
            1.8 if county["county_id"] in ["kisumu", "nakuru", "uasin_gishu"] else 1.2
        )
...
def merge_and_engineer_features(
    climate_df: pd.DataFrame, soil_df: pd.DataFrame, infra_df: pd.DataFrame
) -> pd.DataFrame:
    ...
    print(
        f"  + High risk instances: {merged['high_risk'].sum()} ({merged['high_risk'].mean()*100:.1f}%)"
    )
