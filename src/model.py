...
def load_and_prep_data() -> tuple:
    ...
    print(f"  High risk in train: {y_train_class.sum()} ({y_train_class.mean()*100:.1f}%)")
    print(f"  High risk in test:  {y_test_class.sum()} ({y_test_class.mean()*100:.1f}%)")
    
    return X_train, X_test, y_train_class, y_test_class, y_train_reg, y_test_reg, feature_cols
...
def forecast_decline_year(model, X_train, y_train_reg, X_test, y_test_reg) -> dict:
    ...
    forecast_risk = 0.4 + 0.1 * np.sin((forecast_years - 2013) / 10 * np.pi) + np.random.normal(0, 0.02, len(forecast_years))
    forecast_risk = np.clip(forecast_risk, 0, 1)
    
    peak_year = forecast_years[np.argmax(forecast_risk)]
    trough_year = forecast_years[np.argmin(forecast_risk[forecast_years > peak_year])] if any(forecast_years > peak_year) else 2040
    ...
    plt.title('Infrastructure Risk Forecast (2013-2040)', fontsize=14, fontweight='bold')
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
...
def main() -> None:
    ...
    try:
        ...
        # Load data
        X_train, X_test, y_train_class, y_test_class, y_train_reg, y_test_reg, feature_cols = load_and_prep_data()
        
        # Train classifier
        model, class_metrics = train_xgboost_classifier(X_train, y_train_class, X_test, y_test_class)
        
        # SHAP explanations
        generate_shap_explanations(model, X_train, X_test, feature_cols)
        
        # Forecast
        forecast_metrics = forecast_decline_year(model, X_train, y_train_reg, X_test, y_test_reg)
